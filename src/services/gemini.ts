...
export async function getCuratorResponse(message: string, history: { role: 'user' | 'model', parts: { text: string }[] }[]) {
  try {
    ...
    if (!response.ok) throw new Error("Ошибка прокси");
    ...
    return "Связь с ИИ-Куратором через прокси прервалась.";
  }
}
