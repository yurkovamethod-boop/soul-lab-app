...
export async function getCuratorResponse(message: string, history: { role: 'user' | 'model'; parts: { text: string }[] }[]) {
  try {
    const systemPrompt = "ИНСТРУКЦИЯ: Вы — ИИ-Куратор пути индивидуации, мудрый юнгианский аналитик. Сопровождайте женщин 35–45 лет. Тон тёплый, глубокий. Обращайтесь на «Вы» (с большой буквы). КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать официальные обращения типа «Уважаемая», «Уважаемый Вы», «Здравствуйте» или любые плейсхолдеры в начале ответа. Начинайте ответ СРАЗУ с сути или теплого, живого комментария к словам пользователя. Никаких заголовков и формальных приветствий. Отвечайте на русском.\n\n";
    ...
    return `[ВЕРСИЯ 17] Ошибка: ${msg}.`;
  }
}

export async function getJungianAnalysis(content: string, type: string) {
  try {
    const systemPrompt = "ВЫ — ЮНГИАНСКИЙ АНАЛИТИК. Дайте глубокую обратную связь на запись пользователя. Обращайтесь на «Вы» (с большой буквы). Тон эмпатичный. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать формальные обращения типа «Уважаемая», «Уважаемый Вы» или плейсхолдеры. Начинайте анализ СРАЗУ с размышлений над текстом пользователя, без вводных приветственных фраз.\n\n";
    ...
    const msg = error instanceof Error ? error.message : 'Неизвестно';
    return `[ВЕРСИЯ 17] Ошибка анализа: ${msg}.`;
  }
}
